...
usage() {
  cat <<'EOF'
Usage: flash-esp32-s3-ws.sh [port]

Compiles and uploads the Tier 3 WS ESP32-S3 camera firmware.
Includes mDNS hostname provisioning. Targets ESP32-S3 hardware.

Arguments:
  port    Serial port to use for upload. Default: /dev/ttyACM0

Examples:
  arduino-code/bin/flash-esp32-s3-ws.sh
  arduino-code/bin/flash-esp32-s3-ws.sh /dev/ttyACM1
EOF
}
...
printf 'ESP32-S3 WS flash complete.\n'

...
SKETCH_PATH="$ROOT_DIR/arduino-code/esp32-camera/ESP32_CameraServer_WS/ESP32_CameraServer_WS.ino"
...
usage() {
  cat <<'EOF'
Usage: flash-esp32-wrover-ws.sh [port]

Compiles and uploads the Tier 3 WS ESP32-WROVER camera firmware.
Includes mDNS hostname provisioning. Targets ESP32-WROVER hardware.

Arguments:
  port    Serial port to use for upload. Default: /dev/ttyUSB0

Examples:
  arduino-code/bin/flash-esp32-wrover-ws.sh
  arduino-code/bin/flash-esp32-wrover-ws.sh /dev/ttyUSB1
EOF
}
...
printf 'ESP32-WROVER WS flash complete.\n'
